...
def dashboard(request):
    ...
    CH1 = IntegrityCheck.objects.filter(check_code=1)
    print CH1
    ...
    # if IntegrityCheck.objects.filter(code = 'BQ'):
    #     print "has data"
    # else:
    #     print "no data"
    ...
    for store in Store.objects.all():
        if IntegrityCheck.objects.filter(code = store.code):
            store_status = IntegrityCheck.objects.filter(code = store.code)
            result = not any(check for check in store_status if check.check_code != 0)
        else:
            result = False
        

    return render(request, 'dashboard.html',{'progress' : progress , 'current_slaves': current_slaves, 'overall_slaves': overall_slaves, 'outstanding_slaves': outstanding_slaves })
...
def testStuff():
    ...
    #check if files exists(physically and in codes file)
    with open('C:\\Users\\EWilliams\\Documents\\tcsapp\\tcsapp\\CODES\\storelist', 'r') as stores:
        for store in stores:
            store = store.strip('#').strip()
            #store = Store.objects.filter(code=store)
            QS = NextInvoice.objects.filter(code=store)
            CH = IntegrityCheck.objects.filter(code=store)
            print store+" : "+str(len(QS))
            ...
            for file in glob.iglob(path):
                ...
                if slave > len(QS):
                    CH.create(code=store, slave=slave, check_code=1, message='NOT IN CODES FILE {}'.format(shortname))
                    print shortname, "not in codes__"
                    continue
            ...
            for slave in range(1, len(QS) + 1):
                ...
                if not glob.glob(path):
                    CH.create(code=store, slave=slave, check_code=2, message='NOT FOUND')
                    continue
